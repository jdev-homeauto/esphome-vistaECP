...
// Settings
const char* wifiSSID = "Starfire"; //name of wifi access point to connect to
const char* wifiPassword = "deadbeefbadf00ddeadbeefbad";
const char* accessCode = "1234";  // An access code is required to arm (unless quick arm is enabled)
const char* mqttServer = "192.168.2.175";    // MQTT server domain name or IP address
           ...
           if (vista.extcmd[0]==0x98) {
            ...
            if (vista.extcmd[2]==0xf1 && z > 0 && z <= MAX_ZONES) { // we have a zone status (zone expander address range)
              ...
              if (zones[z].state != zbypass && zones[z].state != zalarm) {
               ...
               sprintf(value_char, "%02x", vista.extcmd[5]);
               Serial.printf("RFX: %s Device State: %s",rf_serial_char,value_char);
               mqttRFPublish(mqttRFTopic,rf_serial_char, value_char);
           }
        }
